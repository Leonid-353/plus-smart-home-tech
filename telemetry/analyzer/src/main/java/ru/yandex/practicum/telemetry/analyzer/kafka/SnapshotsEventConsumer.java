...
public class SnapshotsEventConsumer {
    private final KafkaConsumer<String, SpecificRecordBase> consumer;
    private final KafkaConfigConsumer.ConsumerConfig config;

    @Autowired
    public SnapshotsEventConsumer(KafkaConfigConsumer.ConsumerSnapshotsConfig config) {
    ...
    private void subscribeToTopics() {
        consumer.subscribe(Collections.singletonList(
                config.getTopicName(KafkaConfigConsumer.TopicType.SNAPSHOTS_EVENTS)
        ));
    }
